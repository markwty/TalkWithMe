...
$query = "SELECT ID, Name, Pending FROM login_table WHERE Mobile='${mobile}' AND Password='${md5password}' AND Status=2;";
if ($result = $mysqli->query($query)) {
    if ($row = $result->fetch_assoc()) {
        ...
        if (!$pos && $pos!==0) {
            //opp_id should be in the Pending column
            ...
            $query = "UPDATE login_table SET Pending='{$new_members}' WHERE ID=${id};";
            if ($result2 = $mysqli->query($query)) {
                $query = "UPDATE login_table SET Organisation='' WHERE ID=${opp_id} AND Organisation='${name}';";
